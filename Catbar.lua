...
local function banSpam()
    ...
    for i = 1, 10 do
        ...
        if chatService.ChatVersion == Enum.ChatVersion.TextChatService then
            ...
            if chan then chan:SendAsync(spamMsg) end
        else
            game:GetService("ReplicatedStorage"):WaitForChild("DefaultChatSystemChatEvents"):WaitForChild("SayMessageRequest"):FireServer(spamMsg, "All")
        end
        task.wait(0.1)
    end
end
...
if args[1]:lower() == "/ban" and args[2] then
    -- If sender is Admin and Target is me, and I am not an Admin
